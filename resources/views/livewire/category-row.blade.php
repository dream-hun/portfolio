<tr class="text-left text-slate-900">
    <td class="pl-6 py-4 pr-3 font-medium">{{ $category->name }}</td>
    <td class="pl-4 py-4 text-left hidden sm:table-cell">
        <span class="inline-flex items-center rounded-md bg-green-50 capitalize px-2 py-1 text-xs font-medium text-green-700 ring-1 ring-green-600/20 ring-inset">{{ $category->status }}</span>
    </td>
    ...
    <td class="pl-4 py-4 text-right pr-6 flex gap-2 justify-end">
        <x-menu>
            ...
            <x-menu.items>
                <x-dialog wire:model="categoryEditModal">
                    <x-dialog.open>
                        <x-menu.close>
                            <x-menu.item>
                    ...
                    <x-dialog.panel>
                        <form wire:submit="save" class="flex flex-col gap-4">
                            <h2 class="text-3xl font-bold mb-1">Edit {{$category->name}}</h2>
